...
class Principal(Tk):
    ...
    def insertar(self):
        ...
        con = self.conectar()
        c1 = con.cursor()
        c1.execute("SELECT pro_codigo_k, pro_descripcion FROM productos")
        productos = c1.fetchall()
        con.close()
        opciones = [f"{codigo} - {descripcion}" for codigo, descripcion in productos]
        ...
        Button(self.lf2,text="Registrar",width=30,bg="#bbbbbb").grid(row=3,column=1,pady=30)
